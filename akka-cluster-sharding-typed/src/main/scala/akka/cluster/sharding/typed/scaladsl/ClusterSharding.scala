...
trait ClusterSharding extends Extension { javadslSelf: javadsl.ClusterSharding ⇒
   ...
   * such a role.
   *
   * @tparam M The type of command the entity accepts
...
@DoNotInherit trait EntityRef[M] {
   ...
   */
  def ask[U](f: ActorRef[U] ⇒ M)(implicit timeout: Timeout, scheduler: Scheduler): Future[U]
   ...
   * Please note that an implicit [[akka.util.Timeout]] and [[akka.actor.Scheduler]] must be available to use this pattern.
   */
  def ?[U](message: ActorRef[U] ⇒ M)(implicit timeout: Timeout, scheduler: Scheduler): Future[U] =
